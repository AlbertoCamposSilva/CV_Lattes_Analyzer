...
dt = pd.read_sql(f"select * from indicadores_pareamento where not ano_doutorado is null and grupo in {str(parear_por_grupo).replace('[','(').replace(']',')')}", engine)
...
dt = dt.drop(['chamada', 'programa'], axis=1)
...
for id in financiados.id.unique():
    ...
    if parear_por_area_conhecimento:
        ...
        if to_print: print('     1. Pareando pela área de conhecimento: ', area_para_parear)
        ...
        if tamanho_pareados2 > min_para_parear:
            tipos_de_pareamento.append('area_demanda_bruta')
            pareados = pareados2
            if to_print: print('         Pareado. Tamanho de Pareados: ', tamanho_pareados2)
        else: 
            if to_print: print('         NÃO Pareado. Tamanho de Pareados: ', tamanho_pareados2)
                       
    #2. Pareando pelo Vínculo
    if parear_por_vinculo:
        if to_print: print('     2. Pareando pelo Vínculo')
    ...
    if parear_por_ano_doutorado_20:
        if to_print: print('     3. Pareando por ano de doutorado -> Faixa de 20 anos')
    ...
    if parear_por_ano_doutorado_10:
        if to_print: print('     4. Pareando por ano de doutorado -> Faixa de 10 anos')
        pareados2 = pareados.loc[
    ...
    #5. Pareando por sexo
    if parear_por_sexo:
        if to_print: print('     5. Pareando por sexo')
    ...
    if parear_por_regiao:
        if to_print: print('     6. Pareando por região')
        if tipo_região == 1:
    ...
    if parear_por_erros:
        ...
        if to_print: print('     7. Pareando por erros quadráticos')
        ...
        for index, row in pareados.iterrows():
            ...
            if to_print: print(f'\r         {x}/{len_pareados} - {int(100*x/len_pareados)}% - id: {row.id}. Grupo: {row.grupo}          ', end="", flush=True)
        ...
        desvio_padrão = []
        if to_print: print('         Calculando o Desvio Padrão por id pareado            ')
        if not erro.empty:
    ...
    if to_print: print('     Incluindo no BD')
